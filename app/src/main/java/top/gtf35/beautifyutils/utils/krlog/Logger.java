...
import top.gtf35.beautifyutils.utils.LocalLog;
...
public class Logger implements Printer{
    ...
    private void localLog(StackTraceElement element,int logType, String message,Object... args){
        String[] values = generateValues(element);
        String tag = values[0];
        String fileName = values[1];
        ...
        switch (logType){
            case ERROR:
                localLog.writeLocalLog("等级：ERROR|TAG：" + tag + "| " + message);
                break;
            case VERBOSE:
                localLog.writeLocalLog("等级：VERBOSE|TAG：" + tag + "| " + message);
                break;
            case INFO:
                localLog.writeLocalLog("等级：INFO|TAG：" + tag + "| " + message);
                break;
            case ASSERT:
                localLog.writeLocalLog("等级：ASSERT|TAG：" + tag + "| " + message);
                break;
            case WARN:
                localLog.writeLocalLog("等级：WARN|TAG：" + tag + "| " + message);
                break;
            case DEBUG:
                localLog.writeLocalLog("等级：DEBUG|TAG：" + tag + "| " + message);
                break;

            case OBJECT:
                localLog.writeLocalLog("等级：OBJECT|" + fileName + "| " + TOP_BORDER);
                localLog.writeLocalLog("等级：OBJECT|" + fileName + "| " + HORIZONTAL_DOUBLE_LINE + " " + tag);
                localLog.writeLocalLog("等级：OBJECT|" + fileName + "| " + MIDDLE_BORDER);
                localLog.writeLocalLog("等级：OBJECT|" + fileName + "| " + HORIZONTAL_DOUBLE_LINE + " " + message);
                localLog.writeLocalLog("等级：OBJECT|" + fileName + "| " + BOTTOM_BORDER);
                break;
        }
    }
    ...
    private void printLog(StackTraceElement element,int logType, String message,Object... args){
        ...
        localLog(element,logType, message,args);
